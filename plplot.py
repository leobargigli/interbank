...
def plplot(x,xmin,alpha, ntail, label,  filename,  format = None):
        # select method (discrete or continuous) for fitting
    if     reduce(lambda X,Y:X==True and floor(Y)==float(Y),x,True): 
        f_dattype = 'INTS'
        x = [int(i) for i in x]
    elif reduce(lambda X,Y:X==True and (type(Y)==int or type(Y)==float or type(Y)==long or type(Y)==float32 or type(Y)==float64),x,True):    f_dattype = 'REAL'
    else:                 f_dattype = 'UNKN'
    ...
    plt.ylabel('$\mathrm{P}(X > x)$',fontsize=16);
